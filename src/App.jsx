...
import FestoSmcCrossoverTool from './FestoSmcCrossoverTool';
...
const hasRealImage = (p) => !p.img || !p.img.includes('picsum.photos');
...
}, [products, catFilter, genreFilter]);

const featuredProducts = useMemo(() => {
  const marked = products.filter(p => hasRealImage(p) && (p.new || p.is_new));
  return marked.length > 0 ? marked : products.filter(hasRealImage).slice(0, 6);
}, [products]);
...
{tab==='tryon' && <VirtualTryOn products={products}/>}
{tab==='crossover' && <FestoSmcCrossoverTool/>}
</div>
...
<p className="text-[13px] leading-relaxed mb-4" style={{ color:C.inkSoft }}>{selectedProduct.description || selectedProduct.desc}</p>
)}
